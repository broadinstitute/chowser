...
object VariantIdNew {
  ...
  def apply(chromosome: Chromosome, position: Int, ref: String, alt: String): VariantIdNew =
    VariantIdNew(Location(chromosome, position), ref, alt)
  ...
  def fromVariantContext(context: VariantContext): Seq[Either[String, VariantIdNew]] = {
    Chromosome.parse(context.getContig) match {
      case Left(message) => Seq(Left(message))
      case Right(chromosome) =>
        val pos = context.getStart
        val ref = VariantIdNew.adjustSequenceIfEmpty(context.getReference.getBaseString)
        val alts = context.getAlternateAlleles.asScala.map(_.getBaseString).map(VariantIdNew.adjustSequenceIfEmpty)
        alts.map { alt =>
          Right(VariantIdNew(chromosome, pos, ref, alt))
        }
    }
  }
  ...
  def parse(string: String): Either[String, VariantIdNew] = {
      ...
      Chromosome.parse(parts(0)) match {
        ...
        case Right(chromosome) =>
          ...
          NumberParser.UnsignedIntParser.parseOpt(posString) match {
            ...
            case Some(pos) =>
              val ref = VariantIdNew.adjustSequenceIfEmpty(parts(2))
              val alt = VariantIdNew.adjustSequenceIfEmpty(parts(3))
              Right(VariantIdNew(chromosome, pos, ref, alt))
          }
      }
    }
