...
import chowser.util.io.{FileInputId, OutputId, ResourceConfig}
...
object TsvMatrixExecuter extends ChowserExecuter[TsvMatrixCommand] {
  ...
  override def execute(command: TsvMatrixCommand): Either[Snag, Result] = {
    ...
    idsFile match {
      case FileInputId(idsFileFile) =>
        ...
        val valueReader = BasicTsvReader.forSimpleHeaderLine(valuesFile, resourceConfig, LineParser.whitespace)
        valueReader.foreach { row =>
          matrix.put(row.string(idCol1), row.string(idCol2), row.string(valueCol))
        }
        matrix.writeTo(outFile, resourceConfig)
        Right(Result.Done)
  ...
  class Matrix(ids: Seq[String], default: String, diagonal: String) {
    ...
    def writeTo(file: OutputId, resourceConfig: ResourceConfig): Unit = {
      val writer = TsvWriter(file, TsvHeader.empty, resourceConfig)
